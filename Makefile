# This Makefile is for the SVN::Mirror extension to perl.
#
...
#     ABSTRACT => q[Mirror remote repositories to local subversion repository]
#     AUTHOR => q[Chia-liang Kao <clkao@clkao.org>]
#     DIR => []
#     DISTNAME => q[SVN-Mirror]
#     EXE_FILES => [q[bin/svm]]
#     NAME => q[SVN::Mirror]
#     NO_META => q[1]
#     PL_FILES => {  }
#     PREREQ_PM => { SVN::Simple::Edit=>q[0], URI::Escape=>q[0], SVN::Core=>q[1.0.7], Term::ReadKey=>q[0] }
#     VERSION => q[0.52]
#     clean => { FILES=>q[t/repos* tmp vcp*] }
#     test => { TESTS=>q[t/1use.t t/2basic.t t/3vcp-cvs.t t/4vcp-p4.t t/5upgrade.t t/6relay.t t/7copy.t] }
...
VENDORPREFIX = 
INSTALLPRIVLIB = /usr/local/lib/perl5/5.8.5
DESTINSTALLPRIVLIB = $(DESTDIR)$(INSTALLPRIVLIB)
INSTALLSITELIB = /usr/local/lib/perl5/site_perl/5.8.5
DESTINSTALLSITELIB = $(DESTDIR)$(INSTALLSITELIB)
INSTALLVENDORLIB = 
DESTINSTALLVENDORLIB = $(DESTDIR)$(INSTALLVENDORLIB)
INSTALLARCHLIB = /usr/local/lib/perl5/5.8.5/mach
DESTINSTALLARCHLIB = $(DESTDIR)$(INSTALLARCHLIB)
INSTALLSITEARCH = /usr/local/lib/perl5/site_perl/5.8.5/mach
...
INSTALLMAN3DIR = /usr/local/lib/perl5/5.8.5/perl/man/man3
DESTINSTALLMAN3DIR = $(DESTDIR)$(INSTALLMAN3DIR)
INSTALLSITEMAN3DIR = /usr/local/lib/perl5/5.8.5/man/man3
DESTINSTALLSITEMAN3DIR = $(DESTDIR)$(INSTALLSITEMAN3DIR)
INSTALLVENDORMAN3DIR = 
DESTINSTALLVENDORMAN3DIR = $(DESTDIR)$(INSTALLVENDORMAN3DIR)
PERL_LIB = /usr/local/lib/perl5/5.8.5
...
TO_INST_PM = lib/SVN/Mirror.pm \
	lib/SVN/Mirror/Ra.pm \
	lib/SVN/Mirror/VCP.pm

PM_TO_BLIB = lib/SVN/Mirror/Ra.pm \
	blib/lib/SVN/Mirror/Ra.pm \
	lib/SVN/Mirror.pm \
	blib/lib/SVN/Mirror.pm \
	lib/SVN/Mirror/VCP.pm \
	blib/lib/SVN/Mirror/VCP.pm
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=1 --perm_rw=$(PERM_RW) \
	  bin/svm $(INST_MAN1DIR)/svm.$(MAN1EXT) 
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/SVN/Mirror.pm $(INST_MAN3DIR)/SVN::Mirror.$(MAN3EXT) 
...
$(INST_SCRIPT)/svm : bin/svm $(FIRST_MAKEFILE) blibdirs
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/svm
	$(CP) bin/svm $(INST_SCRIPT)/svm
	$(FIXIN) $(INST_SCRIPT)/svm
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/svm
...
TEST_FILE = test.pl
TEST_FILES = t/1use.t t/2basic.t t/3vcp-cvs.t t/4vcp-p4.t t/5upgrade.t t/6relay.t t/7copy.t
TESTDB_SW = -d
...
ppd:
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>Mirror remote repositories to local subversion repository</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Chia-liang Kao &lt;clkao@clkao.org&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="SVN-Core" VERSION="1,0,7,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="SVN-Simple-Edit" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
...
pm_to_blib.ts: $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')' \
	  lib/SVN/Mirror/Ra.pm blib/lib/SVN/Mirror/Ra.pm \
	  lib/SVN/Mirror.pm blib/lib/SVN/Mirror.pm \
	  lib/SVN/Mirror/VCP.pm blib/lib/SVN/Mirror/VCP.pm 
	$(NOECHO) $(TOUCH) $@
